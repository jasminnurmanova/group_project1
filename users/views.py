...
from .models import Cart, Order, OrderItem, Wishlist, Chat, Message, DepositRequest, BalanceHistory, Promocode, CustomUser
...
class ProfileView(View):
    def get(self, request, username):
        ...
        return render(
            request, "profile.html", {"customuser": user, "products": products}
        )
...
class AddRemoveWishlistView(LoginRequiredMixin, View):
    ...
    def post(self, request, product_id):
        ...
        wishlist_item = models.Wishlist.objects.filter(author=request.user, product=product)

        if wishlist_item.exists():
            wishlist_item.delete()
            messages.info(request, "Wishlistdan olib tashlandi")
        else:
            models.Wishlist.objects.create(author=request.user, product=product)
...
def messenger_inbox(request):
    ...
    all_chats = Chat.objects.filter(Q(user1=sender) | Q(user2=sender)).order_by(
        "-updated_at"
    )
...
def messenger(request, pk):
    ...
    chat = Chat.objects.filter(
        (Q(user1=sender) & Q(user2=recipient)) | (Q(user1=recipient) & Q(user2=sender))
    ).first()
    ...
    Message.objects.filter(chat=chat, is_read=False).exclude(sender=sender).update(
        is_read=True
    )
    all_chats = Chat.objects.filter(Q(user1=sender) | Q(user2=sender)).order_by(
        "-updated_at"
...
class DepositRequestView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Deposit sorovi mufovavaqiyatli yuborildi! Admin tasdiqlashini kuting",
            )
            return redirect("users:deposit")
        user_requests = DepositRequest.objects.filter(
